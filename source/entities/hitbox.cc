...
hitbox_t::hitbox_t(const glm::vec2 &size) : size(size) {
    position = glm::vec2(0.0f);
    std::cout << "[hitbox] Created with size: (" << size.x << ", " << size.y << ")" << std::endl;
}

void hitbox_t::updatePosition(const glm::vec2 &entityPos) {
    position = entityPos;
    std::cout << "[hitbox] Position updated to: (" << position.x << ", " << position.y << ")" << std::endl;
}
...
bool hitbox_t::intersects(const hitbox_t &other) const {
    ...
    const bool isIntersecting = !(left1 > right2 || right1 < left2 || top1 < bottom2 || bottom1 > top2);

    std::cout << "[hitbox] Collision check:" << std::endl;
    std::cout << "  Box 1: pos=(" << position.x << "," << position.y << ") size=(" << size.x << "," << size.y << ")"
              << std::endl;
    std::cout << "  Box 2: pos=(" << other.position.x << "," << other.position.y << ") size=(" << other.size.x << ","
              << other.size.y << ")" << std::endl;
...
glm::vec2 hitbox_t::getCollisionResolution(const hitbox_t &other) const {
    ...
    std::cout << "[hitbox] Resolution vector: (" << resolution.x << ", " << resolution.y << ")" << std::endl;
    return resolution;
}