...
uint32_t TextureManager::getTexture(const string &name) const {
    auto it = sheetMap.find(name);
    return it != sheetMap.end() ? it->second.texID : 0;
}

bool TextureManager::loadSpriteSheet(const string &sheetName, const string &filePath, int texW,
                                     int texH, int sW, int sH) {
...
bool TextureManager::registerSprite(const string &spriteName, const string &sheetName,
    ...
    spriteMap[spriteName] = {sh.texID, sh.texW, sh.texH, sh.sW, sh.sH, frameIndex};
    return true;
}
